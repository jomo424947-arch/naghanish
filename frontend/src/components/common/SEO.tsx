...
export const SEO: React.FC<SEOProps> = ({
  title = 'نغنِش - منصة الألعاب التفاعلية والاختبارات الذكية',
  description = 'نغنِش هي منصتك الترفيهية الفائقة للألعاب الذهنية، اختبارات الشخصية، والبارتي نايت التفاعلي الجماعي مع أصدقائك.',
  keywords = ['نغنش', 'ألعاب ذهنية', 'اختبارات شخصية', 'بارتي نايت', 'العاب اطفال', 'العاب سرعة', 'العاب ذاكرة', 'Naghanish', 'Brain Games', 'Quizzes'],
  image = '/og-image.png',
  url = typeof window !== 'undefined' ? window.location.href : 'https://naghanish.app',
  type = 'website',
  jsonLd,
}) => {
  ...
  useEffect(() => {
    ...
    let scriptJsonLd = document.querySelector<HTMLScriptElement>('script[type="application/ld+json"]')
    if (jsonLd) {
      if (!scriptJsonLd) {
