...
import { Brain, Layers, Zap, Search, Filter, Star, Play } from 'lucide-react'
...
export const GamesPage: React.FC = () => {
  ...
  const filtered = GAMES.filter(g =>
    (selected === 'All' || g.category === selected) &&
    ...
    <div className="flex flex-col gap-8 py-4">
      ...
      <div className="flex items-center gap-3">
        <Input
          ...
          placeholder={dir === 'rtl' ? 'ابحث عن لعبة...' : 'Search games...'}
          value={search}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((game, i) => (
          ...
          >
            <Card variant="glowing" isInteractive padding="none" className="overflow-hidden group">
              {/* Game card header banner */}
              <div className={`relative h-28 bg-gradient-to-br ${game.color} flex items-center justify-center text-5xl`}>
                {game.icon}
                {game.isNew && (
                  <span className="absolute top-3 right-3 rtl:right-auto rtl:left-3 px-2 py-0.5 rounded-full bg-brand-orange text-white text-[10px] font-black shadow">NEW</span>
                )}
              </div>
              <div className="p-4 flex flex-col gap-3">
                <div>
                  <h3 className="font-extrabold text-white text-base">{dir === 'rtl' ? game.titleAr : game.title}</h3>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[11px] font-bold text-slate-400">{game.category}</span>
                    <span className="text-slate-600">•</span>
                    <span className="text-[11px] font-bold text-slate-400">{game.plays} {dir === 'rtl' ? 'لاعب' : 'plays'}</span>
                    <Star className="w-3 h-3 text-amber-400 fill-amber-400 ml-auto rtl:ml-0 rtl:mr-auto" />
